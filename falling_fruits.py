...
while running:
    ...
    fruit_rect=pygame.Rect(fruit_x,fruit_y,fruit_r*2,fruit_r*2)
    if basket_rect.colliderect(fruit_rect):
        score+=1
        fruit_y=0
        fruit_x=random.randint(fruit_r,width-fruit_r)

    if fruit_y -fruit_r > height:
        fruit_y=0
        fruit_x=random.randint(fruit_r,width-fruit_r)

    screen.fill(bg_color)
    pygame.draw.rect(screen, basket_color, (basket_x, basket_y, basket_w, basket_h) )
    pygame.draw.circle(screen,fruit_c,(fruit_x,fruit_y),fruit_r)
